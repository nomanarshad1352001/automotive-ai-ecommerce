...
export function StatCard({ icon, label, value, change, onClick, delay = 0 }: { icon: ReactNode; label: string; value: string | number; change?: string; onClick?: () => void; delay?: number }) {
  const { t } = useTheme();
  return (
    <Card onClick={onClick} className={`p-4 sm:p-5 d${delay}`}>
      <div className="flex items-start justify-between">
        <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center" style={{ background: t.accentBg }}>{icon}</div>
        {change && <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: t.successBg, color: t.success }}>{change}</span>}
      </div>
      <p className="mt-3 text-2xl sm:text-3xl font-extrabold" style={{ color: t.text }}>{value}</p>
      <p className="text-xs sm:text-sm mt-1 font-medium truncate" style={{ color: t.textMuted }}>{label}</p>
    </Card>
  );
}

// Themed Button — bigger
export function Btn({ children, variant = 'primary', className = '', onClick, disabled = false, size = 'md' }: {
  ...
  const { t } = useTheme();
  const base = `inline-flex items-center justify-center gap-1.5 font-bold rounded-xl whitespace-nowrap ${size === 'sm' ? 'px-3 py-1.5 text-xs' : size === 'lg' ? 'px-6 py-3 text-sm' : 'px-4 py-2.5 text-sm'}`;
...
export function Input({ value, onChange, placeholder, className = '', type = 'text' }: {
  ...
  return (
    <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder}
      className={`w-full px-3.5 py-2.5 rounded-xl text-sm outline-none ${className}`}
...
export function Select({ value, onChange, options, className = '' }: {
  ...
  return (
    <select value={value} onChange={e => onChange(e.target.value)}
      className={`w-full px-3 py-2.5 rounded-xl text-sm outline-none cursor-pointer ${className}`}
      style={{ background: t.inputBg, border: `1px solid ${t.inputBorder}`, color: t.text }}>
...
export function Badge({ children, color = 'default' }: { children: ReactNode; color?: 'default' | 'success' | 'warning' | 'danger' | 'info' | 'accent' }) {
  ...
  return <span className="text-xs font-bold px-2.5 py-1 rounded-full inline-flex items-center gap-1 whitespace-nowrap" style={colors[color]}>{children}</span>;
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4" style={{ background: t.modalOverlay }} onClick={onClose}>
      <div className="rounded-t-2xl sm:rounded-2xl border an-scale max-h-[90vh] sm:max-h-[85vh] overflow-y-auto w-full"
        ...
        <div className="flex items-center justify-between p-5 sm:p-6 border-b sticky top-0 z-10 rounded-t-2xl" style={{ borderColor: t.border, background: t.card }}>
          <h2 className="text-base sm:text-lg font-bold" style={{ color: t.text }}>{title}</h2>
          <button onClick={onClose} className="p-2 rounded-lg hover:rotate-90 transition-all" style={{ color: t.textMuted }}
...
export function ProgressBar({ value, color }: { value: number; color?: string }) {
  const { t } = useTheme();
  return (
    <div className="w-full h-2.5 rounded-full overflow-hidden" style={{ background: t.border }}>
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 an-fade">
      <div className="min-w-0">
        <h1 className="text-xl sm:text-2xl font-extrabold" style={{ color: t.text }}>{title}</h1>
        <p className="text-sm mt-1" style={{ color: t.textMuted }}>{subtitle}</p>
      </div>
